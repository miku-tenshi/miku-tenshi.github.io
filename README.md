...
메뉴 글자(`Home`/`Study`/...) 자체도 위 "픽셀 레트로 폰트" 절에서 설명한 `--font-pixel`을
씁니다(`.nav-item`).

## 사이드바가 화면에 붙어있는 이유 (position: sticky)

`.sidebar`는 `position: sticky; top: 0;`으로 화면(뷰포트)에 고정되어 있습니다. 원래는 이
지정이 없어서, 옆의 `.main`(오른쪽 콘텐츠 영역)이 글이 많아 한 화면보다 훨씬 길어지는
페이지(홈의 "최근 글" 목록, study 글 목록 등)에서는 flexbox의 기본 동작(`align-items:
stretch`) 때문에 사이드바도 `.main`만큼 늘어나 버렸습니다. 그러면 사이드바 맨 아래에 있는
"바다"(`.sidebar-sea`)가 페이지 맨 끝까지 밀려나서, 그 페이지를 끝까지 스크롤하지 않으면
사실상 보이지 않는 문제가 있었습니다.

`position: sticky`를 주면 사이드바는 `.main`의 길이와 무관하게 항상 "화면 기준"으로만
움직입니다 — 페이지를 스크롤해도 사이드바는 화면에 붙어있고, 사이드바 자신의 내용(아바타 +
메뉴 + 바다)이 한 화면보다 클 때만 그만큼만 자연스럽게 스크롤됩니다. 아바타 크기·메뉴 글자
크기는 원래 지정된 값 그대로 유지하고(다시 줄이지 않음), 블록 사이 여백(`.sidebar`의
`gap`)과 위아래 `padding`만 조금 줄여서 바다가 들어갈 공간을 최대한 확보했습니다.

- **화면이 작을 때는 여전히 약간의 스크롤이 필요할 수 있습니다**: 아바타(200px)와 메뉴
  5개를 원래 크기 그대로 유지하다 보니, 브라우저 창 세로 길이가 대략 800px보다 작으면
  사이드바 내용 전체가 한 화면에 다 들어가지 않아 바다를 보려면 조금 스크롤해야 합니다.
  다만 이제는 스크롤해야 하는 양이 "페이지 전체 길이"가 아니라 "사이드바 자신의 초과분"
  (보통 100~200px 정도)뿐이라 이전보다 훨씬 금방 나타납니다. 화면 크기와 무관하게 항상
  스크롤 없이 바로 보이게 하고 싶다면, 아바타 크기나 메뉴 항목 `padding`(`.nav-item`)을
  더 줄여야 합니다.
- 모바일(900px 이하, 상단 아이콘 바 레이아웃)에서는 `position: static`으로 되돌아가서
  이 고정 동작이 적용되지 않습니다(원래도 그 레이아웃에서는 필요 없는 동작).
...
설정에서는 자동으로 꺼지고 기본 커서가 그대로 보입니다. **커서 색은 아래 "색상 테마
스위처" 절의 포인트색을 그대로 따라갑니다** — 테마를 바꾸면 커서 점/반짝이 색도 바로 바뀝니다.
...
[Galmuri](https://galmuri.quiple.dev/)라는 오픈소스 한글 픽셀 폰트(`--font-pixel` 변수,
`assets/css/style.css` 맨 위 `@import`로 CDN에서 불러옴)를 **UI/내비게이션 쪽 요소에만**
씁니다 — 사이드바 프로필 이름(`.profile-name`), 메뉴 글자(`.nav-item`, Home/Study/...),
페이지 상단의 폴더 경로 제목(`.main-heading h1`, 예: "study/db/").

**실제로 쓴 글(본문)의 제목·소제목(`.window-body h1`~`h4`)에는 픽셀 폰트를 쓰지 않습니다** —
글을 읽을 때는 원래 기본 폰트(Pretendard)가 더 읽기 편하다는 피드백을 반영해, 카드/전체화면으로
펼쳐지는 글 내용 안의 제목은 본문과 같은 폰트로 되돌렸습니다. 즉 "이 페이지가 어디인지"를
알려주는 UI 요소만 레트로 느낌을 유지하고, "무엇을 읽고 있는지"에 해당하는 실제 글 제목은
가독성을 우선합니다.

- 새로 추가하는 UI 요소에 이 느낌을 주고 싶으면 `font-family: var(--font-pixel);`만 넣으면
  됩니다. 반대로 글 본문 관련 요소에는 넣지 않는 것이 지금의 방향입니다.
...
## 사이드바 아랫부분 — 픽셀 아트 해변 + 밤/새벽/낮/해질녘 (v4)

메뉴 목록 아래 남는 공간(사이드바 아랫단)에는 픽셀 아트 스타일의 해변 풍경이 있습니다
(`.sidebar-sea`, `assets/css/style.css` + `assets/js/sidebar.js`). 하늘(점 텍스처 + 태양/달 +
구름 + 별 + 먼 섬 실루엣) / 바다(파도가 계속 옆으로 흘러가며 치는 거품 파도) / 모래사장 세
구역이 위에서 아래로 쌓여 있고, 실제 방문 시각에 따라 밤·새벽·낮·해질녘 네 가지 색으로 자동
전환됩니다. 왼쪽 아래에는 픽셀 글씨로 된 시계·날짜, 오른쪽 아래에는 저작권 문구가 모래 위에
떠 있습니다. `sidebar.js`가 모든 페이지의 사이드바에 자동으로 넣어주므로 따로 손댈 필요는
없습니다.

실제 이미지 파일을 쓰지 않고 전부 CSS(그라데이션·`clip-path`·`radial-gradient` 반복 타일 등)만
으로 그립니다 — 사이트 전체가 CSS만으로 픽셀 느낌을 내는 기존 방식과 통일하고, 별도 이미지
용량이 들지 않습니다.

- **하늘이 사이드바 배경색과 자연스럽게 이어짐**: `.sea-sky`의 그라데이션이 맨 위는 사이드바
  자체 배경색(`--sidebar-bg`)에서 시작해서 하늘색으로 번져 나가므로, 사이드바 배경과 하늘
  사이에 뚝 끊기는 경계선이 생기지 않습니다.
- **네모 박스가 아니라 사이드바 자체의 여백을 뚫고 나가는 방식(v2부터 유지)**: `.sidebar-sea`는
  사이드바의 좌우/아래 padding만큼 음수 마진(`margin: 0 -56px -48px -56px`)을 줘서, 카드처럼
  떠 있지 않고 사이드바의 진짜 가장자리까지 풍경이 이어지게 만듭니다(이 값은 `.sidebar`의 실제
  padding과 항상 맞춰줘야 합니다).
- **밤/새벽/낮/해질녘 4가지 시간대**: `sidebar.js`가 `new Date().getHours()`로 지금 몇 시인지
  보고 아래 경계로 4구간 중 하나를 판정해서 `.sidebar-sea`에 `data-time="..."` 속성을 붙이고,
  `style.css`의 `.sidebar-sea[data-time="..."]`가 하늘·바다·모래·태양(달)·구름 색을 CSS
  변수(`--sky-a`, `--ocean-a` 등)로 통째로 바꿔치기합니다.
  - 새벽(dawn) 05:00–07:59 · 낮(day) 08:00–16:59 · 해질녘(dusk) 17:00–19:59 ·
    밤(night) 20:00–04:59 — 경계 시각을 바꾸고 싶으면 `sidebar.js`의 `timeSegmentFor()` 함수
    안의 숫자만 고치면 됩니다.
  - 시간대가 바뀔 때 색은 부드럽게 크로스페이드되지 않고 즉시 전환됩니다(그라데이션 색상
    정지점을 애니메이션하는 건 브라우저 지원이 불안정해서 일부러 단순하게 했습니다).
  - 페이지를 오래 열어둔 채로 있어도 1분마다 자동으로 다시 확인하므로, 예를 들어 저녁 8시를
    넘기는 순간 새로고침 없이 밤 풍경으로 바뀝니다.
  - 밤에는 해 대신 달(같은 `.sea-sun` 요소를 재도색), 구름 대신 반짝이는 별이 보입니다.
- **파도가 계속 치는 애니메이션**: `.sea-waves`(두 겹, `w1`/`w2`)는 손으로 그린 완만한 S자
  물결 리본 모양을 인라인 SVG로 만들어 `mask-image`(+ `-webkit-mask-image`)로 잘라내고,
  그 위에 `background-color: var(--foam-color)`로 색을 입힙니다(내비게이션 아이콘
  `.nav-item .nav-icon`과 같은 마스킹 기법 — 이렇게 해야 시간대별 CSS 변수로 거품 색을 계속
  바꿀 수 있습니다). 이 물결 모양을 `mask-position`으로 옆으로 계속 흘려서(`@keyframes
  sea-wave-scroll-w1`/`-w2`) 파도가 끊임없이 밀려오는 것처럼 보이게 합니다. 두 겹의 물결
  모양·두께·속도·방향을 다르게 줘서 겹치는 대신 엇갈리게 보입니다.
  (v4 최초 배포 때는 작은 반원 거품이 반복되는 `radial-gradient` 무늬였는데, 실제로 보니
  파도가 아니라 체크무늬/물방울 무늬처럼 보인다는 피드백을 받아 이 SVG 물결 리본 방식으로
  다시 그렸습니다.)
- **왼쪽 아래 픽셀 시계·날짜**: `.sea-hud` 안의 `.sea-clock`/`.sea-date`에 `sidebar.js`가
  `HH:MM`(24시간제)과 `YYYY.MM.DD` 형식으로 채워 넣고, 1분마다 갱신합니다(`--font-pixel` 사용).
- 모바일(900px 이하, 가로 아이콘 바 형태)에서는 공간이 없어서 이전 버전과 동일하게 전체가
  숨겨집니다(저작권 문구 포함).
- "동작 줄이기(`prefers-reduced-motion`)" 설정에서는 태양/달의 은은한 빛 맥동, 별 반짝임,
  구름 흔들림, 파도 흐름 애니메이션이 전부 멈추고 정지된 장면만 보입니다.
- 색·속도·시간대 경계를 더 바꾸고 싶으면: 색상은 `style.css`의
  `.sidebar-sea[data-time="..."]` 블록들의 CSS 변수, 파도 속도는 `.sea-waves.w1/.w2`의
  `animation` 지속시간, 시간대 경계 시각은 `sidebar.js`의 `timeSegmentFor()` 함수를 고치면
  됩니다.

## 색상 테마 스위처 (사이드바 맨 위 점 5개)

사이드바 맨 위, 프로필 사진 바로 위에 작은 색상 점 5개(`.theme-switcher`)가 있습니다. 클릭하면
그 자리에서 즉시 사이트 전체(배경·글자·포인트색·커서)가 그 테마로 바뀌고, `localStorage`에
저장되어 다음 방문에도 유지됩니다. 방문자별로 각자 기기에 저장되는 개인 취향이라, 같은 사람이
다른 기기로 접속하면 다시 골라야 합니다(계정 개념이 없는 정적 사이트라 서버에 저장하지 않음).

- **테마 5종**: 기본(화이트·블랙·포인트 블루) / 기본 다크 / 블루(연한 파란 그라데이션) /
  민트(파스텔 민트, v4까지의 원래 색상) / 핑크(파스텔 핑크 그라데이션).
- **처음 방문(한 번도 테마를 고른 적 없는 사람)에는 항상 "블루" 테마로 시작합니다**
  (2026-08-26 변경 — 예전에는 5개 중 무작위 배정이었는데, 방문할 때마다/페이지를 옮길 때마다
  색이 계속 바뀌는 것처럼 느껴져 불편하다는 피드백을 받아 고정 기본값으로 바꿈, 아래 "무작위
  배정을 없앤 이유" 절 참고). 한 번이라도 직접 점을 클릭해서 테마를 고르면 그 값이 저장되어
  계속 유지되고, **본인이 직접 다시 클릭하기 전까지는 어떤 페이지/폴더를 옮겨 다녀도 절대
  바뀌지 않습니다.**
- **깜빡임 없이 적용되는 원리**: 모든 페이지의 `<head>` 맨 위(`<meta charset>` 바로 다음)에
  작은 인라인 `<script>`가 있습니다 — 이 스크립트가 `localStorage`를 동기적으로 읽어(없거나
  유효하지 않으면 "blue"로 고정 저장) `<html data-theme="...">` 속성을 CSS가 로드되기도 전에
  붙여둡니다. `assets/css/style.css`의 `:root[data-theme="..."]` 블록들이 이 속성 값에 따라
  배경/글자/포인트색 변수를 통째로 바꿔치기합니다(사이드바 바다의 밤/새벽/낮/해질녘과 똑같은
  원리 — "지금 어떤 테마인지 판정"은 JS가, "그래서 실제 색이 뭔지"는 CSS가 담당).
  `assets/js/sidebar.js`는 페이지가 그려진 뒤에 그 값에 맞춰 점 5개 중 하나를 "활성"으로
  표시하고, 점을 클릭하면 속성을 바꾸고 저장하는 역할만 합니다.
- **마우스 커서도 테마를 따라감**: 커스텀 커서 점(`.custom-cursor`)은 이미
  `background: var(--accent-grad)`를 쓰고 있어서 테마가 바뀌면 저절로 색이 바뀌고, 반짝이는
  잔상(`assets/js/window.js`의 `.cursor-sparkle`)은 예전엔 민트 계열 색 배열이 고정되어
  있었는데, 이제는 마우스가 움직일 때마다 `getComputedStyle`로 그 순간의 `--accent-start`/
  `--accent-end`/`--accent-solid`를 읽어 색을 고릅니다(흰색만 테마와 무관하게 항상 섞임 —
  어느 배경에서도 잘 보이는 중립색이라서).
- **새 페이지를 추가하거나 새 테마를 만들 때 챙길 것(3곳)**: (1) `style.css`에
  `:root[data-theme="새키"]{ ... }` 블록 추가 + `.theme-swatch[data-theme-btn="새키"]`에
  미리보기 그라데이션 추가, (2) `sidebar.js`가 그리는 5개 버튼 목록에 새 버튼 한 줄 추가,
  (3) 모든 페이지 `<head>`의 인라인 스크립트 안 `T` 배열(유효성 검사용)에 같은 키 추가 — 새
  글 발행 시 저장되는 템플릿(`private/write/index.html`의 `PAGE_SKELETON`)도 이 스크립트를
  포함하고 있으니 함께 갱신해야 함. 셋 중 하나라도 빠지면 그 테마만 고를 수 없거나(버튼 없음)
  색이 안 바뀌는 식으로 어긋납니다.
- **무작위 배정을 없앤 이유(2026-08-26)**: "write/memo 창에 들어갈 때마다, 폴더를 옮길 때마다
  배경색이 계속 바뀐다"는 사용자 피드백을 받음. 원인을 찾아보니 사이트 코드 자체의 버그가
  아니라 **배포 시점에 파일 일부만 새로 올라가고 일부는 예전 버전으로 남아있던 것**이 진짜
  원인이었음(예: `sidebar.js`가 테마 스위처 이전 버전으로 한동안 배포돼 있었음 — 아래
  "2026-08-26" project 기록 참고). 다만 그 상태에서 무작위 배정 로직이 함께 있으면 "저장된
  값을 못 찾을 때마다 새로 무작위로 고른다"는 동작 때문에, 배포가 일부 페이지에서 어긋나
  있을 때 증상이 "정말 매번 랜덤하게 바뀌는 것"처럼 보여 원인 파악이 더 헷갈릴 수 있음.
  그래서 "처음 한 번만 무작위, 이후로는 저장된 값 유지"라는 기존 설계 자체는 맞았지만, 사용자
  요청대로 무작위 요소 자체를 완전히 없애고 고정값(blue)으로 바꿔서, 앞으로 비슷한 배포
  불일치가 있더라도 최소한 "이상하게 계속 바뀌는" 것처럼 보이는 일은 없도록 함(코드 어디에도
  테마 관련 `Math.random()` 호출이 더 이상 없음 — `grep -r Math.random`으로 확인 가능한
  커서 반짝이 애니메이션의 문양 선택 등은 테마와 무관하므로 그대로 둠).
- 포인트색 외의 나머지(`--radius`, `--sidebar-w` 등 테마와 무관한 값)는 `:root` 맨 위쪽에
  그대로 있고, 어떤 테마를 골라도 공통으로 적용됩니다.
