...
- **파도가 계속 치는 애니메이션**: `.sea-waves`(두 겹, `w1`/`w2`)가 작은 반원 거품이 옆으로
  늘어선 무늬(`radial-gradient` 타일)를 계속 옆으로 흘려서(`background-position-x` 반복)
  파도가 끊임없이 치는 것처럼 보이게 합니다. 두 겹의 속도·방향·시작 위치를 다르게 줘서 겹치는
  대신 엇갈리게 보입니다.
