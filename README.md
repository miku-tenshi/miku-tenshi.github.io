...
- **처음 방문(한 번도 테마를 고른 적 없는 사람)에는 5개 중 무작위로 하나가 골라집니다** —
  사용자가 "매번 같은 색이라 질린다"고 한 요청 취지에 맞춰, 첫인상부터 매번 다른 색으로
  보이게 함. 한 번 고르거나(직접 클릭) 무작위로 배정되면 그 값이 저장되어 계속 유지됩니다.
- **깜빡임 없이 적용되는 원리**: 모든 페이지의 `<head>` 맨 위(`<meta charset>` 바로 다음)에
  작은 인라인 `<script>`가 있습니다 — 이 스크립트가 `localStorage`를 동기적으로 읽어(없으면
  방금 설명한 무작위 배정) `<html data-theme="...">` 속성을 CSS가 로드되기도 전에 붙여둡니다.
  `assets/css/style.css`의 `:root[data-theme="..."]` 블록들이 이 속성 값에 따라 배경/글자/
  포인트색 변수를 통째로 바꿔치기합니다(사이드바 바다의 밤/새벽/낮/해질녘과 똑같은 원리 —
  "지금 몇 시(또는 어떤 테마)인지 판정"은 JS가, "그래서 실제 색이 뭔지"는 CSS가 담당).
