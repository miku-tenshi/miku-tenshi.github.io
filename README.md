...
`/write/` 페이지가 GitHub Personal Access Token으로 저장소에 직접 커밋하는 관리자용 글쓰기 도구입니다.
`https://miku-tenshi.github.io/write/` 에서 폼에 제목/폴더/본문을 채우고 "GitHub에 올리기"를 누르면
새 글 파일 생성과 폴더 목록(index.html) 업데이트가 한 번에 끝납니다. 토큰을 아직 안 넣었어도 "복사용 코드"
칸에 결과가 채워지므로, 그걸 복사해서 GitHub 웹 화면에서 수동으로 붙여넣어도 됩니다.

- 이 페이지는 메인 메뉴에는 연결되어 있지 않습니다 (검색엔진 노출도 막아뒀습니다). 주소를 기억해두거나 즐겨찾기 해두세요.
- 토큰은 이 브라우저에서 GitHub API로만 바로 전송되며 어디에도 저장되지 않습니다 ("이 브라우저에 토큰 기억하기"를 체크한 경우만 예외).
- 토큰은 이 저장소 하나에만 쓰기 권한이 있는 Fine-grained PAT로 만드는 걸 권장합니다 (`/write/` 페이지 안에 만드는 방법이 안내되어 있습니다).
