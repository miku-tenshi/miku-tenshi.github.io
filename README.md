...
- 왼쪽에 메모 목록(최근 수정 순), 오른쪽에 제목/본문 편집 칸이 있어요.
- "새 메모"로 빈 편집 칸을 열고, "저장"을 누르면 `private/notes/notes.json`에 저장됩니다.
- 목록에서 메모를 클릭하면 편집 칸에 불러와지고, "삭제"로 완전히 지울 수 있어요(되돌릴 수 없어요).
...
- **네모 박스가 아니라 사이드바 자체의 여백을 뚫고 나가는 방식**: `.sidebar-sea`는 사이드바의
  좌우/아래 padding만큼 음수 마진(`margin: 0 -56px -68px -56px`)을 줘서, 카드처럼 떠 있지
  않고 사이드바의 진짜 가장자리까지 색이 이어지게 만듭니다.
- **파도 두 겹**: 서로 다른 색(진한 에메랄드 + 사이트 포인트색)과 속도·방향으로 흐르는 SVG
  물결 두 장을 겹쳐서 물이 실제로 일렁이는 느낌을 줍니다. 두 겹 모두 컨테이너 높이의 100%까지
  꽉 채워서, 사이드바 높이가 달라져도(글 목록이 많아 사이드바가 길어지는 경우 등) 물결과
  바닥색 사이에 이음매(색이 뚝 끊기는 선)가 생기지 않습니다.
  - **SVG를 배경 이미지로 쓸 때 주의할 점**: `background-size`에 두 값(가로/세로)을 명시해도,
    SVG 루트 태그에 `preserveAspectRatio="none"`을 주지 않으면 브라우저가 원래 비율을
    유지하려고 이미지를 가운데로 모으고 여백을 남겨버립니다(letterbox) — 물결이 중간에
    뚝뚝 끊겨 네모난 블록처럼 보이는 원인이었던 버그라, 새로 비슷한 배경을 만들 때도 이
    속성을 꼭 넣어야 합니다.
- 모바일(900px 이하, 가로 아이콘 바 형태)에서는 공간이 없어서 숨겨지고(이때 저작권 문구도
  함께 안 보임 — 원래도 모바일에서는 숨기던 요소라 동작 차이 없음).
- "동작 줄이기(prefers-reduced-motion)" 설정에서는 물결 애니메이션이 멈추고 정지된 모양만
  보입니다.
- 색이나 움직이는 속도, 물결 모양을 바꾸고 싶으면 `.sidebar-sea`의 `background`(바닥색)와
  `::before`/`::after`의 `background-image`(물결 SVG 경로)·`animation` 값을 고치면 됩니다.
